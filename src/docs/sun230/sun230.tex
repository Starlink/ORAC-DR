...
%  History:
%     $Log$
%     Revision 1.24  2004/05/27 20:10:09  bradc
%     updates for ORAC-DR v4.1-0
%
%     Revision 1.23  2004/01/10 00:23:11  bradc
%     SCUBA uses -loop flag instead of -loop wait (in DataLoops section this time)
%
%     Revision 1.22  2004/01/09 23:58:25  bradc
%     add caveat for -loop flag, -loop wait looping options
%
%     Revision 1.21  2003/10/09 02:03:49  bradc
%     add information on ORACDR_PROXY
%
%     Revision 1.20  2003/06/16 00:37:58  mjc
%     It's KAPPA not Kappa.  Correct some of the new oracdr_<instrument> descriptions.
%
%     Revision 1.19  2003/06/13 03:58:01  timj
%     Update for V4.0 release.
%
%     Revision 1.18  2002/09/16 04:30:04  timj
%     Newly translated from V3.1 master and pods
%
...
\subsection*{What it means\label{ORAC-DR_What_it_means}\index{ORAC-DR!What it means}}\begin{description}
\item[pipeline] \mbox{}
...
\section{\xlabel{setting_up_to_run_oracdr}Setting up to run oracdr\label{Setting_up_to_run_oracdr}\index{Setting up to run oracdr}}
...
\subsection*{The short way (Starlink and JAC users only)\label{Setting_up_to_run_oracdr_The_short_way_Starlink_and_JAC_users_only_}\index{Setting up to run oracdr!The short way (Starlink and JAC users only)}}\begin{itemize}
\item 
...
\subsection*{Executables\label{ORAC-DR_Components_Executables}\index{ORAC-DR Components!Executables}}\begin{description}
...
\section{Xoracdr\label{Xoracdr}\index{Xoracdr}\xlabel{xoracdr}}
...
\section{oracdr\label{oracdr}\index{oracdr}\xlabel{oracdr}}
...
\subsubsection*{General Options\label{oracdr_General_Options}\index{oracdr!General Options}}\begin{description}
...
\end{description}
\subsubsection*{\xlabel{windows_and_output}Windows and output\label{oracdr_Windows_and_output}\index{oracdr!Windows and output}}
\begin{description}
...
\subsubsection*{Observations\label{oracdr_Observations}\index{oracdr!Observations}}\begin{description}
...
\end{description}
\subsubsection*{UT date\label{oracdr_UT_date}\index{oracdr!UT date}}\begin{description}
\item[\textbf{-ut}] \mbox{}
...
\end{description}
\subsubsection*{Calibration options.\label{oracdr_Calibration_options_}\index{oracdr!Calibration options.}\xlabel{calibration_options}}
\begin{description}
...
\subsubsection*{\xlabel{looping_options}Looping options\label{oracdr_Looping_options}\index{oracdr!Looping options}}
...
Caution must be used when using the \texttt{-loop wait} option for real-time
data reduction. This looping option examines the file size to figure out if
a file has finished writing so it can be used for reduction. As some systems
allocate the required size on disk before fully writing the data, the pipeline
can get confused and throw errors. For these types of systems (i.e. SCUBA, CGS4)
it is recommended that a flag file be written when the data file has been 
fully written, and the \texttt{-loop flag} option be used instead.

See the \emph{DataLoops} manpage for more
information on looping schemes.

\subsubsection*{Group processing options\label{oracdr_Group_processing_options}\index{oracdr!Group processing options}}\begin{description}
\item[\textbf{-batch}] \mbox{}
...
\end{description}
\subsubsection*{Engine Options\label{oracdr_Engine_Options}\index{oracdr!Engine Options}}\begin{description}
...
\section{\xlabel{data_loops}The ORAC-DR Data Loops\label{The_ORAC-DR_Data_Loops}\index{The ORAC-DR Data Loops}}
...
\section{\xlabel{display_system}The ORAC-DR Display System\label{The_ORAC-DR_Display_System}\index{The ORAC-DR Display System}}
...
\item \textbf{image (\texttt{GAIA}, \texttt{KAPVIEW}, \texttt{P4})}
...
\section{\xlabel{calibration_selection}The ORAC-DR Calibration Selection\label{The_ORAC-DR_Calibration_Selection}\index{The ORAC-DR Calibration Selection}}
...
\section{Shell Variables\label{Shell_Variables}\xlabel{shell_variables}\index{Shell Variables}}
...
If set, all intermediate files created by \oracdr\ will be kept.
This does not include temporary files; to keep temporary files
run \oracdr\ with the \texttt{-debug} flag.

\item[\texttt{ORACDR\_PROXY}] \mbox{}

If set, \oracdr\ will use a proxy for network lookups. This variable
...
\subsection*{Notes\label{oracdr_nuke_NOTES}\index{oracdr nuke!NOTES}}\begin{itemize}
\item 
