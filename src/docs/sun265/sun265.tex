...
\sstroutine{
   ...
   \sstdescription{
      ...
      output files have a suffix of \_cal.
   }
   \sstnotes{
...
\sstroutine{
   ...
   \sstdescription{
      A simple PICARD recipe to trim images from SCUBA-2. The image may
      be trimmed using given spatial dimensions or a statistical
      estimator to threshold the image. In the former case, the output
      map is rectangular or circular; in the latter the shape is
      determined by the values in the image itself.
      ...
      parameters. Note that if a map radius is given, in order to
      produce a circular output image, the height and width are ignored.
   ...
   \sstdiylist{
      ...
      \sstsubsection{
         ...
         Method to use for cropping image. May be statistical or
         spatial, each of which may be abbreviated to the first four
         characters.
      }
      \sstsubsection{
         MAP\_HEIGHT
...
\sstroutine{
   ...
   \sstdescription{
      This recipe reads the corresponding quick-look log file for NEPs,
      and calculates the RMS and NEFD to compare with the integration
      time calculator (ITC).
...
\sstroutine{
   ...
   \sstdescription{
      ...
      REDUCE\_SCAN\_FAINT\_POINT\_SOURCES\_JACKKNIFE (or one of its aliases)
      that have had the artificial gaussian added at the map-making
      stage (most likely ending \_mappsf.
...
\sstroutine{
   ...
   \sstdescription{
      A PICARD recipe to calculate the azimuthally-average spatial power
      spectrum of a SCUBA-2 map. The map is trimmed to a given size
      (rectangular or circular) before having the source emission masked
      out. The power spectrum is calculated and smoothed with a 5-pixel
      boxcar. The peak and half-power points are determined and reported
      to the user in terms of angular scale on the sky. Finally the mean
      power on various scales is calculated and reported from the
      beamsize up to the length scale defined by the filtering when the
      map was made.
   }
   \sstnotes{
      \sstitemlist{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      reference as an initial guess, and the tangent-plane X,Y offset
      between the peak and reference positions is calculated. This
      offset is applied to the WCS. Note that only a linear shift is
...
\sstroutine{
   ...
   \sstdescription{
      A simple PICARD recipe to undo the applied SCUBA-2 calibration.
      The units of the input data are checked and either the FCF in the
      FITS header (if present) or the appropriate default FCF is chosen.
      The output files have a suffix of \_uncal.
   }
   \sstdiylist{
      Available Parameters
   }{
