...
raw/ufti/YYYYMMDD/}, and {\tt reduced/ufti/YYYYMMDD/}\footnote{Prior
to 2000 August these were {\tt ufti\_data/YYYYMMDD/raw/}, and {\tt
ufti\_data/YYYYMMDD/reduced/} respectively.} for the corresponding
reduced data.  So if your data are stored in {\tt
/home/users/abc/data/UKIRT/raw/ufti/20001108/} you should enter
the following.
...
data array and dynamic headers, such as the start time of the exposure,
and another for static headers.\footnote{Prior to 2000 August IRCAM raw
NDFs were copied to {\tt \$ORAC\_DATA\_OUT}, and UFTI's raw \FITSref\
files were converted to NDFs in this directory.  To process such data
you should invoke {\bf oracdr\_$<$instrument$>$\_old} instead of
{\bf oracdr\_$<$instrument$>$}.}  Each container file
is converted to a single NDF in {\tt \$ORAC\_DATA\_OUT} with a merged
set of headers.  Your current directory remains unchanged.
...
17000 for IRCAM in STARE or NDSTARE mode, and 33000 for IRCAM using
...
Observation Tool will reduce the frequency of such errors, they will
not be eliminated.
...
\newpage
\appendix
\section{\xlabel{UFTI_file_suffices}File suffices}\label{UFTI_file_suffices}%
...
\multicolumn{3}{c}{\large{\bf Fabry-Perot and polarimetry recipes}} \vspace*{1ex} \\
\hline
Recipe Name & Type of Data & Function and Comments \\ \hline
...
except the number of jitter points is fixed.  The JITTER9\_SELF\_FLAT
reduces a nine-point jitter.
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         were {\tt{f}} and {\tt{rg}} respectively.
...
\sstroutine{
   ...
   \sstnotes{
      \sstitemlist{
         ...
         that of the aperture (7.8 arcsec); the outer annulus is 2.5 times 
         (15 arcsec) for UFTI and twice the aperture (12 arcsec) for IRCAM.
...
\sstroutine{
   ...
   \sstdescription{
      This script reduces a Fabry-Perot observation with UFTI or IRCAM data.
   ...
   \sstnotes{
      \sstitemlist{
         ...
         \sstitem
         You may use \htmlref{SKY\_FLAT\_FP}{SKY\_FLAT\_FP} to make the flat field.
...
\sstroutine{
   ...
   \sstnotes{
      \sstitemlist{
         ...
         times that of the aperture (7.8 arcsec); the outer annulus is 2.5
         times (15 arcsec) for UFTI and twice the aperture (12 arcsec) for
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         were {\tt{f}} and {\tt{rg}} respectively,. and $<$filter$>$ is the filter
         name.
...
\sstroutine{
   ...
   \sstnotes{
	 ...
	 diameter is 1.3 times that of the aperture (7.5 arcsec); the
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         were {\tt{f}} and {\tt{rg}} respectively.  Token {\tt$<$beam$>$} is {\tt{e}} or {\tt{o}};
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         where {\tt$<$m$>$} is {\tt{gf}} for UFTI and {\tt{gi}} for IRCAM.  Before 2000 August these
         were {\tt{f}} and {\tt{rg}} respectively.  Token $<$beam$>$ is {\tt{e}} or {\tt{o}};
...
\sstroutine{
   ...
   \sstnotes{
	 ...
	 column.
         ...
         \sstitem
         The photometry uses the mode calculated from 3 * median - 2 * mean
	 and Chauvenet's rejection criterion to estimate the sky level
	 in an annulus about the source.  The inner annulus diameter is
	 1.3 times that of the aperture (7.8 arcsec); the outer
	 annulus is 2.5 times (15 arcsec) for UFTI and twice the
	 aperture (12 arcsec) for IRCAM. The errors are internal,
	 based on the sky noise.
...
\newpage
\section{\xlabel{se_changes}Release Notes---V2.0-0\label{se_changes}}
...
\begin{itemize}
   ...
   \item \htmlref{Improved registration}{automatic_registration} allowing
      ...
      separations---for combining polarimetry mosaics.
   ...
   \item The names of flats have changed.  The filter name is included
      for easy identification.  The {\tt{\_cycle}} suffix is shortened to
      {\tt{\_c}}, and all {\tt{\_subgrp}} strings removed from the group
      number.  Flats are are not combined over multiple cycles over
      recipes.  SKY\_FLAT and SKY\_FLAT\_MASKED are limited to jitters
      between three to five points for compatibility with other ORAC
      tools.  It is still possible to make private variants of these
      recipes in which the number of jitter positions is set by the
      NUMBER steering parameter.
