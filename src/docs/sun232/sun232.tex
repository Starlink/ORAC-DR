...
%  Copyright:
%     Copyright (C) 1998--2002 Particle Physics and Astronomy
...
approximately 0.4\% of UFTI pixels, 0.1\% of IRCAM and IRIS2 pixels, and a
...
The aperture size is only 5 arcseconds.  The background is determined
...
\htmlref{NOD\_CHOP\_FAINT}{NOD\_CHOP\_FAINT} & &
   As NOD\_CHOP, but performs a median filter to form an image of the source
   with four times the signal, which is then smoothed to enhance the visibility
   of faint sources. \\ \hline
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         The corrected frame in $<$i$>$$<$date$>$\_$<$obs\_number$>$\_raw,
         where $<$i$>$ is {\tt "}f{\tt "} for UFTI, {\tt "}m{\tt "} for Michelle,  and {\tt "}i{\tt "} for IRCAM. For IRIS2, the corrected frame is in $<$date$><$obs\_number$>$, where
         $<$date$>$ is in the form {\tt{ddmmm}}.
         If the file already exists, it{\tt '}s only updated.
      }
   }
   \sstdiytopic{
      Copyright
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         is {\tt{gf}} for UFTI and {\tt{gi}} for IRCAM and IRIS2.  Before 2000 August these
         ...
         IRCAM frames had prefix {\tt{ro}}. IRIS2 data are in {\tt$<$date$><$obs\_number$>$\_ff},
         where $<$date$>$ is in the format {\tt{ddmmm}}.
      }
   }
   \sstparameters{
      \sstsubsection{
...
.}{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         The individual flat-fielded frames in {\tt$<$i$>$$<$date$>$\_$<$obs\_number$>$\_ff},
         where {\tt$<$i$>$} is {\tt{f}} for UFTI and {\tt{i}} for IRCAM.  Before 2000 August
         IRCAM had prefix {\tt{ro}}. IRIS2 data are in {\tt$<$date$><$obs\_number$>$\_ff},
         where {\tt$<$date$>$} is in the form {\tt{ddmmm}}.
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         {\tt$<$m$>$} is {\tt{gf}} for UFTI and {\tt{gi}} for IRCAM and IRIS2.  Before 2000 August these
         were {\tt{g}} and {\tt{rg}} respectively,. and {\tt$<$filter$>$} is the filter
         name.
...
\sstroutine{
   ...
   \sstdescription{
      ...
      variance, difference the integrations for each AB chop beam pair,
      bad-pixel masking, difference adjacent nodded pairs, registers the
      frames, and forms a mosaic.  See the {\tt "}Notes{\tt "} for further information.
      ...
      calculated.  The results appear in \$ORAC\_DATA\_OUT/aphot\_results.txt in
   ...
   \sstnotes{
      \sstitemlist{
         ...
         which has its bad pixels filled and is then added into a master
         mosaic of improving signal to noise.  The exposure time is also
         summed and stored in the mosaic{\tt '}s corresponding header.  Likewise
         the end airmass and end UT headers are updated to match that of the
         last-observed frame contributing to the mosaic.
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         The integrated mosaic in $<$m$>$$<$date$>$\_$<$group\_number$>$\_mos, where $<$m$>$
         is {\tt "}gm{\tt "} for Michelle.

         \sstitem
         A mosaic for each cycle of object frames in
         $<$m$>$$<$date$>$\_$<$group\_number$>$\_mos$<$cycle\_number$>$, where $<$cycle\_number$>$
         counts from 0.

         \sstitem
         The combined source image and neighbourhoods in
         $<$m$>$$<$date$>$\_$<$group\_number$>$\_cab.

         \sstitem
         The differenced pairs in $<$i$>$$<$date$>$\_$<$obs\_number$>$\_dp,
         where $<$i$>$ is {\tt "}m{\tt "} for Michelle.
      }
   }
   \sstdiytopic{
   ...
   \sstdiytopic{
      ...
      NOD\_CHOP, NOD\_SELF\_FLAT\_NO\_MASK\_APHOT, BRIGHT\_POINT\_SOURCE\_APHOT,
      JITTER\_SELF\_FLAT\_APHOT,
   }
   \sstdiytopic{
      Copyright
...
\sstroutine{
   ...
   \sstdescription{
      ...
      See the {\tt "}Notes{\tt "} for further information.
   }
   \sstnotes{
      \sstitemlist{
         ...
         from centroiding on bright sources with recipe NOD\_CHOP\_APHOT.
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         The combined source image and neighbourhoods in
         $<$m$>$$<$date$>$\_$<$group\_number$>$\_cab.  The smoothed combined image in
         $<$m$>$$<$date$>$\_$<$group\_number$>$\_scab.
   ...
   \sstdiytopic{
      ...
      NOD\_CHOP, NOD\_CHOP\_APHOT, NOD\_SELF\_FLAT\_NO\_MASK.
   }
   \sstdiytopic{
      Copyright
   }{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      variance, difference the integrations for each AB chop beam pair,
      bad-pixel masking, difference adjacent nodded pairs, registers the
      frames, and forms a mosaic at each scan position.  See the {\tt "}Notes{\tt "}
      for further information.
   }
   \sstnotes{
      \sstitemlist{
         ...
         the mosaic{\tt '}s corresponding header.  Likewise the end airmass and end
         UT headers are updated to match that of the last-observed frame
         contributing to the mosaic.
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         The integrated mosaics in $<$m$>$$<$date$>$\_$<$group\_number$>$\_o$<$scan$>$\_mos,
         where $<$m$>$ is {\tt "}gm{\tt "} for Michelle, $<$group\_number$>$ is the number of
         group, and $<$scan$>$ is the index number of the distinct scan
         position counting from 0.

         \sstitem
         A mosaic for each cycle of object frames in
         $<$m$>$$<$date$>$\_$<$group\_number$>$\_o$<$scan$>$\_mos$<$cycle\_number$>$, where $<$cycle\_number$>$
         counts from 0.
...
\sstroutine{
   ...
   \sstdescription{
      ...
      normalisation by the mean of the image. Files the normalised
...
\begin{description}
  ...
  \item [ARRAY\_TESTS]
    ...
    a log file, and the readnoise is filed with the calibration system.
  \item [\htmlref{DARK\_SUBTRACT}{DARK\_SUBTRACT}]
    Subtracts a dark frame from an observation. This recipe is meant
    for a quick look for data that is being taken at a high rate.
  \item [\htmlref{NOD\_CHOP\_FAINT}{NOD\_CHOP\_FAINT}]
    Reduction of nodded and chopped data, specifically for Michelle.
    It is similar to NOD\_CHOP, but forms a single image of the source
    using a median filter, then smooths the combined image to enhance
    the visibility of faint sources.
...
\begin{itemize}
  \item \htmlref{NOD\_CHOP\_APHOT}{NOD\_CHOP\_APHOT}~~
    The photometry aperture has been increased to 5 arcseconds from 3, and
    inner and outer diameters of the sky annulus have been modified correspondingly.
\end{itemize}
