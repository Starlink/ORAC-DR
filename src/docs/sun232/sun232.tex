...
%     2004 May 27 (BRADC):
%        232.8: Updated for v4.1. Included v4.1 Release Notes.
...
The recipes with a \_CATALOGUE suffix (currently only JITTER\_SELF\_FLAT\_CATALOGUE) create a source catalogue from the mosaic. This catalogue is
created using \EXTRACTOR\ and includes objects having at least 12
connected pixels above 1.0 $\sigma$ above sky. The catalogue is
written in ARK Cluster format,
with a field ID in column 0 (this value is typically zero), an object ID
in column 1, right ascension in columns 2, 3, and 4, declination in columns
5, 6, and 7, x and y positions in columns 8 and 9, uncalibrated magnitude
in column 10, error in magnitude in column 11, and quality flag in
column 12 (this value is typically zero). The magnitudes are given by
the expression $-2.5 \lsk$log10(counts) per second exposure time.

As the final mosaic has varying noise characteristics, with higher noise
regions at the edges, the detection limit varies across the mosaic. Fainter
objects can be detected in the central region than near the edges and
corners.
\newline {\em[\_CREATE\_SOURCE\_CATALOGUE\_]}
...
\sstroutine{
   ...
   \sstdescription{
      ...
      This recipe performs a null debiassing, bad-pixel masking, dark
      subtraction, flat-field division, feature detection and matching
      between object frames, and resampling.  See the \htmlref{``Notes''}{bpsna_notes} for details.
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         is {\tt{gf}} for UFTI and {\tt{gi}} for IRCAM (before 2000 August these
         were {\tt{g}} and {\tt{rg}} respectively), also {\tt{gi}} for IRIS2, and {\tt{gu}} for UIST, and {\tt$<$filter$>$} is the filter name.
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         The integrated mosaic in {\tt$<$m$>$$<$date$>$\_$<$group\_number$>$\_$<$filter$>$\_mos},
         where {\tt$<$m$>$} is {\tt{gf}} for UFTI and {\tt{gi}} for IRCAM (before 2000
         August these were were {\tt{g}} and {\tt{rg}} respectively), {\tt{gi}} also
         for IRIS2, {\tt{gu}} for UIST, {\tt{gisaac}} for ISAAC, and {\tt{gingrid}} for INGRID, and {\tt$<$filter$>$} is the filter name.

         \sstitem
         A mosaic for each cycle of jittered frames in\\
         {\tt$<$m$>$$<$date$>$\_$<$group\_number$>$\_$<$filter$>$\_mos$<$cycle\_number$>$}, where {\tt$<$cycle\_number$>$}\\
         counts from 0.
