...
%     CONVERT, CURSA, GAIA, KAPPA, PHOTOM, and PISA.  Each
%     command is \package.
...
%     arcsecond, and underscore.

%  Things to do:
%     - Remark about using -verbose.
%     - Acknowledgment to Starlink.
%     - Mention logfiles.
%     - Example of -calib.
...
%  Copyright:
%     Copyright (C) 1998--2000 Particle Physics and Astronomy
...
\newcommand{\stardocnumber}    {232.1}
\newcommand{\stardocauthors}   {Malcolm J. Currie\\
                               Joint Astronomy Centre, Hilo, Hawaii}
\newcommand{\stardocdate}      {2000 February 2}
...
\section{\xlabel{Using_the_pipeline}Using the
pipeline\label{Using_the_pipeline}}

\subsection{Setting up \ORACDR}
...
For example, the standard location for raw UFTI data is
{\tt ufti\_data/YYYYMMDD/raw/}, and {\tt ufti\_data/YYYYMMDD/reduced/}
for the corresponding reduced data.  So if your data are stored in
{\tt /home/users/abc/data/UKIRT/ufti\_data/19991108/raw/} you should
enter the following.

\begin{verbatim}
      % setenv ORAC_DATA_ROOT  /home/users/abc/data/UKIRT/
      % oracdr_ufti 19991108
\end{verbatim}
to enable the pipeline for UFTI data taken on 1999 November 8.
...
the earlier example with UFTI for UT date 1999 November 8,
{\tt \$ORAC\_DATA\_IN} points to {\tt \$ORAC\_DATA\_ROOT/ufti\_data/19991108/raw/}.

\ORACDR\ operates in {\tt \$ORAC\_DATA\_OUT}, irrespective of
what your current directory is when you invoke it.  Raw data are
either copied (IRCAM) to {\tt \$ORAC\_DATA\_OUT}, or raw \FITSref\
files are converted to NDFs (UFTI) in this directory.  Your current
...
\begin{verbatim}
      % oracdr -list 41:49,51:59 JITTER9_SELF_FLAT
\end{verbatim}
processes frames from 41 to 49 inclusive and 51 to 59 inclusive,
invoking the {\tt JITTER9\_SELF\_FLAT} recipe, and overriding
the RECIPE header.
...
is to include a dark frame accidently.  Check the log in the raw data
directory; it has file extension {\tt .nightlog}.  If you do not have
a log, it is easy to create one.

\begin{verbatim}
      % oracdr -noeng -from 1 -skip NIGHT_LOG
\end{verbatim}
...
to do most of the data processing. All but two of the imaging recipes are
...
of your final mosaics.  Otherwise the pipeline becomes sometime of 
...
is adequate for most uses of UFTI and IRCAM/TUFTI.  For older
...
like a few counts, but over longer integrations or in the thermal
regime they can amount to a few score.
...
recipes assume that you have used their corresponding `execs', and
hence have not changed the exposure time during a jitter.  The
exposure time (header EXP\_TIME) is propagated from the first frame to
the mosaic.  Where multiple frames combined to create a mosaic pixel
the signal-to-noise ratio corresponds to the combined integration
time.  The integration time (keyword INT\_TIME) is the number of
...
A mosaic forms for each cycle of the recipe, e.g. all nine frames in
a JITTER9\_SELF\_FLAT.  For multiple cycles, an integrated `grand'
mosaic forms of improving signal-to-noise.  To avoid the build up of
bad pixels from cosmic rays, bad pixels are interpolated before the
...
are bad in all frames and should be ignored.  The EXP\_TIME for the
integrating mosaic is the sum of the exposure times of the
contributing mosaics.  Again the signal is not divided by the exposure
time.
...
depending on the recipe.  The annulus is typically 9 to 15 arcseconds
...
The magnitudes are given by the expression $-2.5 * $log10(abs(counts)
...
entries in a table provides a catalogue magnitude in $J$, $H$, or $K$
for a standard star.  Also a mean extinction is applied for the mean
...
At the time of writing, the {\em \ORACDR\ Programmer's Guide\/} User
Note is unavailable.  For most purposes observers wishing to make or
modify existing scripts can get by without this document.
...
The first of these primitives is either \_UFTI\_HELLO\_ or \_IRCAM\_HELLO\_.
They contain instrument-specific code and initialisation.  Those are
best left alone.  See 
\begin{htmlonly}
...
does not keep track of the errors per pixel.  If you wish to know
...
\subsubsection{\xlabel{steering_primitives}Steering primitive}
...
\begin{verbatim}
    cd $ORAC_RECIPE_DIR
    cp $ORAC_DIR/recipes/UFTI/JITTER9_SELF_FLAT ./JITTER7_SELF_FLAT
\end{verbatim}
...
\end{verbatim}
substituting {\tt 7} for the {\tt 9}.  The recipe will then generate
the self flat, flat field and make the mosaic once the seventh frame is
dark-subtracted.

\subsubsection{\xlabel{recipe_primitives}Recipe primitives}
...
\begin{verbatim}
#FILTER MODE ORACTIME RDOUT_X1 RDOUT_X2 RDOUT_Y1 RDOUT_Y2
flat_23 Lp98 STARE 13.3484 1 256 1 256
flat_436 J98 flush_read 1.40859E+01 1 1024 1 1024
flat_78 K98 flush_read 7.23825E+00 1 1024 1 1024
flat_88 J98 flush_read 7.46111E+00 1 1024 1 1024
flat_93 H98 flush_read 7.58330E+00 1 1024 1 1024
flat_98 K98 flush_read 7.70559E+00 1 1024 1 1024
flat_133_subgrp H98 flush_read 8.99034E+00 1 1024 1 1024
flat_138_subgrp H98 flush_read 9.07139E+00 1 1024 1 1024
flat_138_subgrp_cycle1 H98 flush_read 9.12074E+00 1 1024 1 1024
flat_290_row0 K98 flush_read 1.13094E+01 1 1024 1 1024
flat_290_row1 K98 flush_read 1.15080E+01 1 1024 1 1024
\end{verbatim}
...
the common ones is to make an error in the `exec'.  This can cause
frames to be in the wrong observation groups or be assigned the wrong
...
\item [{\tt GRPMEM}]---the sequence number within the group, counting
from 1.
\end{description}
Three of them should obey the following relationship:
${\tt GRPNUM} = {\tt OBSNUM} - {\tt GRPMEM} + 1$.
...
\begin{verbatim}
      % fitsmod f19991108_00042 grpnum update 36 \$C !
\end{verbatim}
...
the UFTI scripts, and for subsequent discussions.  Thanks also to
Frossie Economou and Tim Jenness for answering my \ORACDR\ and Perl
questions, and for incorporating my requested enhancements into
\ORACDR.

\section{Copyright and License}

\ORACDR\ is copyright \copyright 1998--2000 PPARC (the UK Particle Physics
...
\newpage
\appendix
\section{\xlabel{UFTI_file_suffices}UFTI file
suffices}\label{UFTI_file_suffices}%
\index{ufti_file_suffices}

\subsection*{Description}%
\index{Description}

Files generated during UFTI data reduction have suffices denoting the
processing step that created them.  This appendix contains a list with
short descriptions of what they mean.  Most will be removed once a
recipe has finished using them.  So you will probably only see these
files if you list the contents of directory {\tt \$ORAC\_DIR} while the
pipeline is running, or you interrupt the pipeline with {\tt CTRL/C},
or something has gone wrong with the the recipe and pipeline has
aborted.

\subsection*{UFTI Frame suffices}%
...
{\tt\_db} & De-Biassed        & The bias is actually zero, but it sets
                                up various \xref{\CCDPACK}{sun139}{} things
...
{\tt\_nl} & Non-Linearity     & The standard non-linearity correction has
                                been applied \\
{\tt\_nm} & Normalised to Mode& Created in \htmlref{QUADRANT\_JITTER}{QUADRANT_JITTER}
                                to normalise frames combined to make the flat field \\
...
are for IRCAM/TUFTI.  The magnitude ranges are courtesy of Sandy
Leggett.
\bigskip\bigskip
...
\htmlref{SKY\_FLAT}{SKY\_FLAT} & Flat &
   ...
   Mostly for use with BRIGHT\_POINT\_SOURCE recipes. \\ \hline
...
\begin{center}
\begin{tabular}{|l|p{24mm}|p{59mm}|}
\multicolumn{3}{c}{\large{\bf Very-bright-point-source recipes}} \vspace*{1ex} \\
\hline
Recipe Name & Type of Data & Function and Comments \\ \hline
\htmlref{BRIGHT\_POINT\_SOURCE}{BRIGHT\_POINT\_SOURCE} & 
   \mbox{$I$,$Z<13$}, \mbox{$JHK<9$}, and bright \mbox{$13<I$,$Z<17$}, \mbox{$9<JHK<15$} & 
   5-point jitter but would be useable as 3-point.  Requires a
   separate flat as the background is too low to self flat. \\ \hline
...
\newpage
\begin{center}
\begin{tabular}{|l|p{25mm}|p{59mm}|}
\multicolumn{3}{c}{\large{\bf Bright-point-source recipes}} \vspace*{1ex} \\
\hline
Recipe Name & Type of Data & Function and Comments \\ \hline
\htmlref{JITTER3\_SELF\_FLAT}{JITTER3\_SELF\_FLAT} &
   \mbox{$13<I$,$Z<17$}, \mbox{$9<JHK<15$} & 3-point standard jitter, self flats.
   Not recommended. \\ \hline
\htmlref{JITTER5\_SELF\_FLAT}{JITTER5\_SELF\_FLAT} & &
   5-point standard jitter, self flats. \\ \hline
\htmlref{JITTER5\_SELF\_FLAT\_APHOT}{JITTER5\_SELF\_FLAT\_APHOT} & &
   As JITTER5\_SELF\_FLAT, but also performs aperture photometry of the
   source. \\ \hline
\htmlref{JITTER5\_SELF\_FLAT\_NO\_MASK}{JITTER5\_SELF\_FLAT\_NO\_MASK} & &
   As JITTER5\_SELF\_FLAT but faster as it lacks object masking.  It
   only suitable for uncrowded fields. \\ \hline
\htmlref{SKY\_AND\_JITTER5}{SKY\_AND\_JITTER5} & &
   A sky frame and 5-point jitter.  The sky is subtracted from the
   target frame before flat fielding.  Requires a
   separate flat as the background is too low to self flat.
   No longer recommended as sky varies too quickly. \\ \hline
\htmlref{SKY\_AND\_JITTER5\_APHOT}{SKY\_AND\_JITTER5\_APHOT} & &
   As SKY\_AND\_JITTER5, but also performs aperture photometry of the
   source.  \\ \hline
\end{tabular}
\end{center}
\bigskip\bigskip
...
\hline
Recipe Name & Type of Data & Function and Comments \\ \hline
\htmlref{JITTER9\_SELF\_FLAT}{JITTER9\_SELF\_FLAT} & Faint
   $I$,$Z>17$, $JHK>15$ & 9-point standard jitter, self flats. Otherwise
   known as JITTER\_SELF\_FLAT. \\ \hline
\htmlref{JITTER9\_SELF\_FLAT\_BASIC}{JITTER9\_SELF\_FLAT\_BASIC} & &
   Fastest JITTER9\_SELF\_FLAT recipe as it lacks object masking, automatic
   registration and resampling. \\ \hline
\htmlref{JITTER9\_SELF\_FLAT\_NO\_MASK}{JITTER9\_SELF\_FLAT\_NO\_MASK} & &
   As JITTER9\_SELF\_FLAT, but faster as it lacks object masking.  It
   only suitable for uncrowded fields. \\ \hline
\end{tabular}
\end{center}
...
\newpage
\begin{center}
\begin{tabular}{|l|p{24mm}|p{61mm}|}
\multicolumn{3}{c}{\large{\bf Point-source recipes---thermal}} \vspace*{1ex} \\
\hline
Recipe Name & Type of Data & Function and Comments \\ \hline
\htmlref{NOD4\_SELF\_FLAT\_NO\_MASK}{NOD4\_SELF\_FLAT\_NO\_MASK} & Bright $L<10$ &
   Nod 4-point jitter, self flats of differenced pairs of frames.  Has superior
   and fast sky subtraction.  No object masking. \\ \hline
\htmlref{NOD4\_SELF\_FLAT\_NO\_MASK\_APHOT}{NOD4\_SELF\_FLAT\_NO\_MASK\_APHOT} & &
   As NOD4\_SELF\_FLAT\_NO\_MASK, but also performs aperture photometry of the
   positive and negative sources.  \\ \hline
\htmlref{NOD8\_SELF\_FLAT\_NO\_MASK}{NOD8\_SELF\_FLAT\_NO\_MASK} & Faint $L>10$, all $M$ &
   Nod 8-point jitter, self flats of differenced pairs of frames.  Has superior
   sky subtraction.  No object masking. \\ \hline
\htmlref{NOD8\_SELF\_FLAT\_NO\_MASK\_APHOT}{NOD8\_SELF\_FLAT\_NO\_MASK\_APHOT} & &
   As NOD8\_SELF\_FLAT\_NO\_MASK, but also performs aperture photometry of the
   positive and negative sources.  \\ \hline
\end{tabular}
\end{center}
...
\htmlref{QUADRANT\_JITTER\_BASIC}{QUADRANT\_JITTER\_BASIC} & &
   Fastest QUADRANT\_JITTER recipe as it lacks object masking, automatic
   registration and resampling. \\ \hline
\htmlref{EXTENDED\_3x3}{EXTENDED\_3x3} & Galaxies and nebulae with
   angular extent $<$2 arcminutes & Sky-subtracted 3$\times$3 grid
   mosaic.  Frames alternate between sky and target. \\ \hline
...
\hline
Recipe Name & Type of Data & Function and Comments \\ \hline
\htmlref{MOVING\_JITTER9\_SELF\_FLAT}{MOVING\_JITTER9\_SELF\_FLAT} &
   Minor planets, comets &
   As JITTER9\_SELF\_FLAT, but uses ephemeris data to track the non-sidereal source. \\ \hline
\htmlref{MOVING\_JITTER9\_SELF\_FLAT\_BASIC}{MOVING\_JITTER9\_SELF\_FLAT\_BASIC} &  &
   As JITTER9\_SELF\_FLAT\_BASIC, but uses ephemeris data to track the non-sidereal source.\\ \hline
\htmlref{JITTER9\_SELF\_FLAT\_TELE}{JITTER9\_SELF\_FLAT\_TELE} & &
   9-point standard jitter, using telescope offsets.  This is needed
   when the telescope has tracked on the non-sidereal target. \\ \hline
\end{tabular}
\end{center}

\subsection{Reference documentation}

The reference documentation which follows describes the UFTI recipes.
Since the IRCAM recipes are virtually identical, to list these, when
the UFTI recipes already have considerable repetition, was
profiligate. The differences to note are the frames have the {\tt ro}
prefix instead of {\tt f}, the names of group frames, i.e. mosaics,
begin with {\tt rg} instead of {\tt g} for UFTI.  The fields of view
as given in recipes like EXTENDED\_5x5 will be correspondingly
smaller, depending on the plate scale.  For IRCAM3 data without the
magnifier, the field of view is 72 arcseconds, compared with 91
arcseconds for UFTI's full-size frame.  IRCAM data will usually have a
...
\sstroutine{
   ...
   Calculates the ND\_STARE readout noise and dark current
}{
   \sstdescription{
      This script calculates the STARE and ND\_STARE readout noises,
      and the dark current from a series of engineering data taken
      with the EXEC called ARRAY\_TESTS.  The results are compared with
   ...
   \sstnotes{
      \sstitemlist{
         ...
         \sstitem
         The engineering log contains the UT date and time, the ND\_STARE
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         The engineering log: {\tt /ukirt\_sw/logs/ufti\_array\_tests.log}.
      }
   }
   \sstimplementationstatus{
      \sstitemlist{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      with UFTI data.  It takes an UFTI observation comprising a
      series of object frames and a dark frame, and a predetermined
   ...
   \sstnotes{
      \sstitemlist{
         ...
         \sstitem
         You may use SKY\_FLAT or SKY\_FLAT\_MASKED to make the flat field.
         ...
         \sstitem
         For each cycle of five, the recipe creates a mosaic, which
         ...
         EXP\_TIME header.  Likewise the end airmass header, AMEND, is
         updated to match that of the last-observed frame contributing to
         the mosaic.

         \sstitem
         Intermediate frames are deleted except for the flat-fielded
         ({\tt\_ff} suffix) frames.
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         The resultant mosaic in {\tt g$<$date$>$\_$<$group\_number$>$\_mos}.
   ...
   \sstparameters{
      \sstsubsection{
         ...
         The number of frames in the jitter pattern. {\tt[5]}
      }
   }
   \sstdiytopic{
      Timing
...
\sstroutine{
   ...
   \sstdescription{
      ...
      appear in {\tt\$ORAC\_DATA\_OUT/aphot\_results.txt} in the form of 
      a Starlink \xref{small text list}{sun190}{STLREF}.  The analysis
      of each star is appended to this file.
   ...
   \sstnotes{
      \sstitemlist{
         ...
         For each cycle of five, the recipe creates a mosaic, which
         is then added into a master mosaic of improving signal to noise.
         The exposure time is also summed and stored in the mosaic{\tt '}s
         EXP\_TIME header.  Likewise the end airmass header, AMEND, is
         updated to match that of the last-observed frame contributing to
         the mosaic.
...
\sstroutine{
   ...
   \sstnotes{
      \sstitemlist{
         ...
         stored in the mosaic's EXP\_TIME header.  Likewise the end airmass
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         The full mosaic in {\tt g$<$date$>$\_$<$group\_number$>$\_mos}.

         \sstitem
         A mosaic for each row in {\tt g$<$date$>$\_$<$group\_number$>$\_mos$<$row\_number$>$},
...
\sstroutine{
   ...
   Reduces a 3-point ``standard jitter'' photometry observation using
   object masking
}{
   \sstdescription{
      This script reduces a ``standard jitter'' photometry observation
      with UFTI data.  It takes an UFTI observation comprising three
   ...
   \sstnotes{
      \sstitemlist{
         ...
         \sstitem
         For each cycle of three, the recipe creates a mosaic, which has
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         The integrated mosaic in {\tt g$<$date$>$\_$<$group\_number$>$\_mos}.

         \sstitem
         A mosaic for each cycle of four in
         {\tt g$<$date$>$\_$<$group\_number$>$\_mos\_$<$cycle\_number$>$},
         where {\tt$<$cycle\_number$>$} counts from 0.

         \sstitem
         The individual flat-fielded frames in {\tt f$<$date$>$\_$<$obs\_number$>$\_ff}.

         \sstitem
         The created flat fields in {\tt flat\_$<$group\_number$>$subgrp} for the
         first or only cycle, and {\tt flat\_$<$group\_number$>$subgrp\_$<$cycle\_number$>$}
   ...
   \sstparameters{
      \sstsubsection{
         ...
         The number of frames in the jitter pattern. {\tt[3]}
      }
   }
   \sstdiytopic{
      Related Recipes
...
\sstroutine{
   ...
   Reduces a 5-point ``standard jitter'' photometry observation using
   object masking
}{
   \sstdescription{
      This script reduces a ``standard jitter'' photometry observation
      with UFTI data.  It takes an UFTI observation comprising five
      object frames and a dark frame to make a calibrated, untrimmed
      mosaic automatically.
   ...
   \sstnotes{
      \sstitemlist{
         ...
         \sstitem
         For each cycle of five, the recipe creates a mosaic, which has
         its bad pixels filled and is then added into a master mosaic of
         improving signal to noise.  The exposure time is also summed and
         stored in the mosaic's EXP\_TIME header.  Likewise the end airmass
         header, AMEND, is updated to match that of the last-observed frame
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         A mosaic for each cycle of four in
         {\tt g$<$date$>$\_$<$group\_number$>$\_mos\_$<$cycle\_number$>$}, where {\tt$<$cycle\_number$>$}
         counts from 0.
   ...
   \sstdiytopic{
      ...
      \htmlref{JITTER5\_SELF\_FLAT\_APHOT}{JITTER5\_SELF\_FLAT\_APHOT},
      \htmlref{JITTER5\_SELF\_FLAT\_NO\_MASK}{JITTER5\_SELF\_FLAT\_NO\_MASK},\\
      \htmlref{JITTER9\_SELF\_FLAT}{JITTER9\_SELF\_FLAT},
      \htmlref{JITTER9\_SELF\_FLAT\_NO\_MASK}{JITTER9\_SELF\_FLAT\_NO\_MASK},
      \htmlref{QUADRANT\_JITTER}{QUADRANT\_JITTER},\\
      \htmlref{QUADRANT\_JITTER\_NO\_MASK}{QUADRANT\_JITTER\_NO\_MASK}.
   }
...
\sstroutine{
   ...
   \sstdescription{
      ...
      \xref{small text list}{sun190}{STLREF}.  The analysis of each star is appended to this file.
   ...
   \sstimplementationstatus{
      \sstitemlist{
         ...
         \sstitem
         The processing engines are from the Starlink packages: \xref{\CCDPACK}{sun139}{},
         \xref{\KAPPA}{sun95}{}, \xref{\PISA}{sun109}{}, and \xref{\PHOTOM}{sun45}{}.
...
\sstroutine{
   ...
   \sstdescription{
      This script reduces a ``standard jitter'' photometry observation
      with UFTI data.  It takes an UFTI observation comprising nine
      object frames and a dark frame to make a calibrated, untrimmed
      mosaic automatically.
   ...
   \sstnotes{
      \sstitemlist{
         ...
         \sstitem
         For each cycle of nine, the recipe creates a mosaic, which has
         its bad pixels filled and is then added into a master mosaic of
         improving signal to noise.  The exposure time is also summed and
         stored in the mosaic's EXP\_TIME header.  Likewise the end airmass
         header, AMEND, is updated to match that of the last-observed frame
   ...
   \sstparameters{
      \sstsubsection{
         ...
         The number of frames in the jitter pattern. {\tt[9]}
      }
   }
   \sstdiytopic{
      Related Recipes
...
\newpage
\sstroutine{
   JITTER9\_SELF\_FLAT\_NO\_MASK
}{
   Reduces a 9-point ``standard jitter'' photometry observation
}{
   \sstdescription{
      This script reduces a ``standard jitter'' photometry observation
      with UFTI data.  It takes an UFTI observation comprising nine
...
\sstroutine{
   ...
   \sstdescription{
      This script reduces a ``standard jitter'' photometry observation
      with UFTI data.  It takes an observation comprising nine
      object frames and a dark frame to make a calibrated, untrimmed
      mosaic automatically.
...
\sstroutine{
   ...
   Reduces a 9-point ``standard jitter'' photometry observation of a
   ...
   \sstdescription{
      ...
      target using ephemeris data stored in file {\tt target\_ephem.txt}.
      ...
      should be performed by \htmlref{JITTER9\_SELF\_FLAT\_TELE}{JITTER9\_SELF\_FLAT\_TELE} which registers
   ...
   \sstnotes{
      \sstitemlist{
         ...
         ORAC\_EPHEMERIS, defaulting to {\tt \$ORAC\_DATA\_OUT/target\_ephem.dat}.
   ...
   \sstdiytopic{
      ...
      \ssthitemlist{
         ...
         \sstitem
           the motion in the plane of the sky in arcseconds/second for right
...
\sstroutine{
   ...
   \sstdescription{
      ...
      of pixel origin to register to fixed sky co-ordinates.  The
      registration is adjusted to track the motion of the moving
      target using ephemeris data stored in file {\tt target\_ephem.txt}.
      See \htmlref{``Ephemeris-file Format''}{ephem_format} for details
      of this file's format.
...
\sstroutine{
   ...
   \sstnotes{
      \sstitemlist{
         ...
         \sstitem
         Run with {\tt oracdr -noeng -from 1 -skip}  for efficiency.
...
\newpage
\sstroutine{
   NOD4\_SELF\_FLAT\_NO\_MASK
}{
   Reduces a 4-point ``nod jitter'' observation
}{
   \sstdescription{
      This script reduces a ``nod jitter'' observation with UFTI data.  It
      takes an UFTI observation comprising four object frames and a dark
      frame to make a calibrated, untrimmed mosaic automatically.
   ...
   \sstdiytopic{
      ...
      \htmlref{NOD4\_SELF\_FLAT\_NO\_MASK\_APHOT}{NOD4\_SELF\_FLAT\_NO\_MASK\_APHOT},
      \htmlref{NOD8\_SELF\_FLAT\_NO\_MASK}{NOD8\_SELF\_FLAT\_NO\_MASK},\\
      \htmlref{NOD8\_SELF\_FLAT\_NO\_MASK\_APHOT}{NOD8\_SELF\_FLAT\_NO\_MASK\_APHOT}.
   }
   \sstimplementationstatus{
      \sstitemlist{
...
\newpage
\sstroutine{
   NOD4\_SELF\_FLAT\_NO\_MASK\_APHOT
}{
...
\newpage
\sstroutine{
   NOD8\_SELF\_FLAT\_NO\_MASK
}{
   Reduces a 8-point ``nod jitter'' observation
}{
   \sstdescription{
      This script reduces a ``nod jitter'' observation with UFTI data.
      It takes an UFTI observation comprising eight object frames and a
   ...
   \sstdiytopic{
      ...
      \htmlref{NOD4\_SELF\_FLAT\_NO\_MASK\_APHOT}{NOD4\_SELF\_FLAT\_NO\_MASK\_APHOT},
      \htmlref{NOD4\_SELF\_FLAT\_NO\_MASK}{NOD4\_SELF\_FLAT\_NO\_MASK},\\
      \htmlref{NOD8\_SELF\_FLAT\_NO\_MASK\_APHOT}{NOD8\_SELF\_FLAT\_NO\_MASK\_APHOT}.
   }
   \sstimplementationstatus{
      \sstitemlist{
...
\newpage
\sstroutine{
   NOD8\_SELF\_FLAT\_NO\_MASK\_APHOT
}{
   Reduces a 8-point ``nod jitter'' photometry observation,
   and performs aperture photometry
}{
   \sstdescription{
      This script reduces a ``nod jitter'' photometry observation with
      UFTI data.  It takes an UFTI observation comprising eight
      object frames and a dark frame to make a calibrated, untrimmed
      mosaic automatically.
...
\sstroutine{
   ...
   \sstdescription{
      ...
      e.g. a quasar.  The object need not be isolated, as the recipe
      masks objects within the other quadrants, and hence does not
      introduce significant artifacts into the flat field.  For isolated
      objects use \htmlref{QUADRANT\_JITTER\_NO\_MASK}{QUADRANT\_JITTER\_NO\_MASK}; or where speed is critical,
      use QUADRANT\_JITTER\_BASIC instead.
   }
   \sstnotes{
      \sstitemlist{
...
\sstroutine{
   ...
   \sstdescription{
      ...
      object.  This variant of \htmlref{QUADRANT\_JITTER}{QUADRANT\_JITTER} is best for isolated
      objects or where speed is critical.  Use \htmlref{QUADRANT\_JITTER}{QUADRANT\_JITTER} itself
...
\sstroutine{
   ...
   \sstdescription{
      ...
      introduce artifacts into the flat field.  Use \htmlref{QUADRANT\_JITTER}{QUADRANT\_JITTER} for
      those.
   }
   \sstnotes{
      \sstitemlist{
...
\sstroutine{
   ...
   \sstdescription{
      This recipe reduces a dark frame observation with UFTI
      data.  It files the dark in file {\tt\$ORAC\_DATA\_OUT/index.dark}.
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         The dark is filed in {\tt f$<$\$ORAC\_DATA\_OUT/index.dark$>$}.
      }
   }
   \sstdiytopic{
      Timing
   }{
      Of the order of a few seconds on an Ultra 5.
   }
   \sstimplementationstatus{
      \sstitemlist{
...
\sstroutine{
   ...
   Reduces a 5-point ``combined jitter'' photometry observation, and
...
\sstroutine{
   ...
   \sstdescription{
      ...
      \htmlref{SKY\_FLAT\_MASKED}{SKY\_FLAT\_MASKED} will
   ...
   \sstnotes{
      \sstitemlist{
         ...
         \sstitem
         The bad pixel mask applied is \$ORAC\_DATA\_CAL/bpm.
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         The flat is called {\tt flat\_$<$group\_number$>$}.
...
\sstroutine{
   ...
   Creates and files a flat field derived from 5 jittered frames using
   ...
   \sstdescription{
      ...
      It performs a null debiassing, bad-pixel masking, and dark subtraction
      before combining the object frames frames pixel by pixel to make the flat.
      See the notes for further details.  The parameters of the flat is filed
