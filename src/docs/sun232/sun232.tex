...
\newcommand{\stardocnumber}    {232.5}
\newcommand{\stardocauthors}   {Malcolm J. Currie\\
                               Joint Astronomy Centre, Hilo, Hawaii}
\newcommand{\stardocdate}      {2001 November 23}
...
Michelle instruments.  It outlines the algorithms used and how to make
...
Michelle.  It outlines the various algorithms used in the recipes, and
...
file}{sun92}.  For UFTI and IRCAM they comprise one NDF for the data
...
choose a \KAPPA\ GWM widget.  Most people prefer a simple scaled image
display with \GAIA.  This offers facilities to inspect and analyse the
data, and both pixel and sky co-ordinates of the cursor position are
presented.  The selection of frame types to display, where they should
appear, and how they are scaled are configurable using a simple text
file or a special GUI tool {\bf oracdisp}.  See
...
follows: UFTI 26$e^{-}$ and IRCAM 47$e^{-}$.  Michelle reductions
...
headers do not provide a sky co-ordinate system.  Using information in
...
fields of view.  \GAIA\ and \KAPPA\ can display these co-ordinates on
overlay grids and axes.  For all these, such as {\KAPPA}'s {\bf
display} with axes, you may need to select the appropriate astrometric
Frame like this.
...
approximately 0.4\% of UFTI pixels, 0.1\% of IRCAM pixels, and a
...
The variability of IRCAM and Michelle bad pixels is unknown at the
...
33000 using the Deepwell; and 100000 for Michelle.  The lower limit is
...
jitter.  The exposure time (header EXP\_TIME [UFTI/Michelle] or
...
The integration time (keyword INT\_TIME [UFTI] or EXPOSED [IRCAM]) is
...
The EXP\_TIME or DEXPTIME for the integrating mosaic is the sum of the
...
diameters are 6.5 to 10 arcseconds (IRCAM), and 6.5 to 12.5 arcseconds
...
The aperture size is only 3 arcseconds.  The background is determined
from every pixel within the combined-image frame, except for a
6-arcsecond circle enclosing the source.  The Michelle chip
...
{\tt$<$instrument$>$} is {\tt UFTI}, {\tt MICHELLE}, or {\tt IRCAM};
and there a few general scripts in {\tt \$ORAC\_DIR/primitives/general}.
...
For old UFTI data, you can edit the raw FITS files.  {\tt
...
which create the data variance.  Thanks also to Frossie Economou and
Tim Jenness for answering my \ORACDR\ and Perl questions, and for
incorporating my requested enhancements into \ORACDR.
...
Michelle.  For IRCAM, dimensions are 22\% smaller.  The magnitude
ranges are courtesy of Sandy Leggett.
\bigskip\bigskip
...
\htmlref{NOD\_CHOP}{NOD\_CHOP} &
   ...
   giving a mosaic with two positive and two negative images.  No object
   masking or flat-fielding.  Requires a bias in CHOP mode. \\ \hline
...
\htmlref{MOVING\_NOD\_CHOP}{MOVING\_NOD\_CHOP} & &
   As NOD\_CHOP, but uses ephemeris data to track the non-sidereal source.\\ \hline
...
\sstroutine{
   ...
   \sstdescription{
      This recipe reduces a moderately extended source using UKIRT imaging
...
\sstroutine{
   ...
   \sstdescription{
      This recipe reduces an extended source using UKIRT imaging data.
...
\sstroutine{
   ...
   \sstparameters{
      ...
      \sstsubsection{
         ...
         there are NPAIRS pairs of frames.  A value of 1 also dictates
         that no jittering has occurred.  To make a master mosaic combining
         spatial positions, NUMBER should be at least 3.  {\tt[]}
      }
      \sstsubsection{
         USEVAR = LOGICAL
      }{
...
\sstroutine{
   ...
   \sstdescription{
      This script reduces a ``standard jitter'' photometry observation
      with UKIRT imaging data observed through one or more filters.  For
...
\sstroutine{
   ...
   \sstnotes{
      \sstitemlist{
         ...
         \sstitem
         The bias removed from each beam is currently a null bias, so the
         errors will be overestimated in the CHOP read mode.  The data
         array will be unaffected once the beams are differenced.  Using
         a bias frame awaits the ARRAY\_TESTS recipe to file a suitable
...
\sstroutine{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         The integrated mosaic in {\tt$<$m$>$$<$date$>$\_$<$group\_number$>$\_mos}, where {\tt$<$m$>$}
         is {\tt{gf}} for UFTI and {\tt{gi}} for IRCAM.  Before 2000
         August these were {\tt{g}} and {\tt{rg}} respectively.

         \sstitem
         A mosaic for each cycle of four in \\
         {\tt$<$m$>$$<$date$>$\_$<$group\_number$>$\_mos$<$cycle\_number$>$}, where {\tt$<$cycle\_number$>$}\\
         counts from 0.  Before 2000 August IRCAM frames had prefix {\tt{ro}}.
...
\sstroutine{
   ...
   \sstdescription{
      ...
      It performs a null debiassing, creation and propagation of data
      variance, difference the integrations for each AB chop beam
      pair, bad-pixel masking, difference adjacent nodded pairs,
      registers the frames, and forms a mosaic.  See the
      \htmlref{``Notes''}{nca_notes} for further information.
      ...
      to form a single image with four times the signal.  Photometry
      of the point source using a fixed 3-arcsecond aperture is then
      calculated.  The results appear in {\tt \$ORAC\_DATA\_OUT/aphot\_results.txt}
      in the form of a Starlink small text list.  The analysis of each
      star is appended to this file.
    }
    \label{nca_notes}
    \sstnotes{
       \sstitemlist{
         ...
         The photometry uses a multiply clipped (2,2,2.5,3 standard
         deviations) mean to estimate the sky mode in an annulus about the
         source.  The inner annulus diameter is 2.0 times that of the
         aperture (6 arcsec); the outer annulus is 3.0 times (9 arcsec)
...
\sstroutine{
   ...
   \sstdescription{
      This recipe reduces a dark-frame observation with UKIRT imaging
...
\newpage
\section{\xlabel{se_changes3p0}Release Notes---V3.0-1\label{se_changes3p0}}
\markboth{{\stardocname}~ --- Release Notes---V3.0-1}
{{\stardocname}~--- Release Notes---V3.0-1}
...
\begin{itemize}
   ...
   \item Easier to switch off data variance processing.  This combined
   ...
   \item Better formatting of output with blank lines to block related
         output, and some of the commentary contain more details.
   ...
   \item The mosaics have the world co-ordinate system domain set to sky
         so that displays with \GAIA\ and \KAPPA\ have sky co-ordinates.
   \item The end UT for mosaics is updated to that of the
         last-contributing frame.
...
\begin{itemize}
   \item The primitives utilise user headers for steering.
