...
%  Copyright:
%     Copyright (C) 2014 Las Cumbres Observatory Global Telescope Network Inc.
%     All Rights Reserved.
...
\newcommand{\stardoccopyright} {Copyright \copyright\ 2014 Las Cumbres Observatory Global Telescope Inc.}
...
\sstroutine{
   ...
   \sstnotes{
      \sstitemlist{
         ...
         The upper limit is 1000 above the saturation limit for the
         detector in the mode used.
   ...
   \sstdiytopic{
      ...
      {\tt "}Scripts for UFTI{\tt "} G.S. Wright \& S.K. Leggett, 1997 orac009-ufts,
      v01.
   }
   \sstdiylist{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         The dark called dark\_$<$exposure\_time$>$\_$<$frame\_number$>$. For single
         frames the group number is the same as the frame number. The
         decimal point in the time is replaced by {\tt "}p{\tt "}.

         \sstitem
         The dark is filed in \$ORAC\_DATA\_OUT/index.dark.
      }
   }
   \sstimplementationstatus{
      \sstitemlist{
...
\sstroutine{
   ...
   \sstdescription{
      This recipe makes a sky flat for UKIRT imaging from a series of
      sky or object frames which are combined using one of a selection
      of statistics. It expects one dark frame followed by jittered sky
      frames.
      ...
      future selection and use of the flat. See the {\tt "}NOTES{\tt "} for further
      details.

      For best results the field observed should contain few stars and
      no bright ones. In contaminated sky regions, recipe
      SKY\_FLAT\_MASKED will greatly reduce artifacts appearing in the
      resultant flat.
   }
   \sstnotes{
      \sstitemlist{
   ...
   \sstdiytopic{
      ...
      \sstitemlist{
         ...
         \sstitem
         The created flat fields in flat\_$<$filter$>$\_$<$group\_number$>$ for the
         first or only cycle, and
         flat\_$<$filter$>$\_$<$group\_number$>$\_c$<$cycle\_number$>$ for subsequent recipe
         cycles. Token $<$filter$>$ is the filter name, $<$group\_number$>$ is the
         frame number of the group, and $<$cycle\_number$>$ is the number of the
         cycle, counting from one.
   ...
   \sstdiytopic{
      ...
      SKY\_FLAT\_FP, SKY\_FLAT\_MASKED, SKY\_FLAT\_POL.
   }
   \sstimplementationstatus{
      \sstitemlist{
