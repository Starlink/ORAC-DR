...
\sstroutine{
   ...
   \sstdescription{
      ...
      \sstitemlist{
         ...
         to the timeseries, to create a signal map and a map-filtered psf
         image.

         \sstitem
         These images are coadded (like with like) to produce a total
         signal map and a total map-filtered psf image. The amplitude of
         the source in the map-filtered PSF image is compared with the
         input value to assess the effect the map-making process has on a
         point source. This ratio is used to scale the FCF later when
         ...
         low-spatial frequency noise from the signal map and the
         map-filtered psf. This is the so-called whitening step (because it
         produces a map which has a noise power spectrum that is white).
      ...
      be two for each observation, a total signal coadd, a map-filtered
      psf coadd, a jackknife map
   }
   \sstnotes{
      \sstitemlist{
...
\sstroutine{
   ...
   \sstdescription{
      This recipe creates an image from a POINTING observation with the
      ...
      corrected for extinction. SCAN-mode data are passed to the
      iterative map-maker. Fast-ramp flatfield files are processed and
      stored so the iterative map-maker can use them.

      The image is calibrated and a flag flag file written to allow the
      POINTING\_FOCUS task at the summit to derive pointing offsets. The
      image is cropped to 60 arcsec on a side and any residual
...
\sstroutine{
   ...
   \sstdescription{
      ...
      sky at 850 um as possible, beginning with two strips: one along
      the Galactic Plane, one perpendicular to the Plane, and passing
      through the North Ecliptic Pole.
...
\sstroutine{
   ...
   \sstdescription{
      ...
      image, which is calibrated and displayed. FCFs are derived if the
      source is a calibrator. A new Group image is created and displayed
      after all the individual observations have been processed. The
      noise properties of the new Group image are calculated and written
      to a log file, log.noise.
