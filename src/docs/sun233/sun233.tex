...
\newcommand{\stardocnumber}    {233.2}
\newcommand{\stardocauthors}   {Tim Jenness, Frossie Economou\\
Joint Astronomy Centre, Hilo, Hawaii}
\newcommand{\stardocdate}      {1 Apr 2001}
...
The following headers can be used. Note that not all headers are defined
for each instrument.
...
Airmass at start of integration.

\item[AIRMASS\_END] \mbox{}

Airmass at end of integration.
...
Configuration index. Increments when instrument hardware configuration
changes.

\item[DEC\_BASE] \mbox{}

Offset zero-point declination at equinox (in degrees).

\item[DEC\_SCALE] \mbox{}

Co-ordinate increment along declination axis (typically in degrees/pixel).

\item[DEC\_TELESCOPE\_OFFSET] \mbox{}

Telescope offset declination at equinox (in arcseconds).
...
Equinox of object position (in years).
...
Camera mode. Used for multi-mode instruments like Michelle.
...
Observation type. Used to determine whether observation is of
OBJECT, DARK, or ARC, for example.

\item[RA\_BASE] \mbox{}

Right ascension of base position {in hours}

\item[RA\_SCALE] \mbox{}

Coordinate increment along right ascension axis (typically in
hours/pixel)

\item[RA\_TELESCOPE\_OFFSET] \mbox{}

Telescope offset right ascension at equinox (in arcseconds).

\item[ROTATION] \mbox{}

Angle of declination axis with respect to axis 2, measured counterclockwise
(in degrees).
...
UTC date when observation was made.
...
Polarimeter waveplate angle (in degrees).
