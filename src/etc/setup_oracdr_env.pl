...
my %envs = ( 'ACSIS'  => { ORAC_DATA_CAL => File::Spec->catfile( $ENV{'ORAC_CAL_ROOT'}, "acsis" ),
             ...
             'OCGS4'    => { ORAC_DATA_CAL => File::Spec->catfile( $ENV{'ORAC_CAL_ROOT'}, "cgs4" ),
                             ORAC_DATA_ROOT => $dataroot,
                             ORAC_DATA_IN => File::Spec->catfile( $dataroot, "raw", "cgs4", $ut ),
                             ORAC_DATA_OUT => File::Spec->catfile( $dataroot, "reduced", "cgs4", $ut ),
                             ORAC_PERSON => 'phirst',
             ...
             'UFTI2'    => { ORAC_DATA_CAL => File::Spec->catfile( $ENV{'ORAC_CAL_ROOT'}, "ufti" ),
                             ORAC_DATA_ROOT => $dataroot,
                             ORAC_DATA_IN => File::Spec->catfile( $dataroot, "raw", "ufti", $ut ),
                             ORAC_DATA_OUT => File::Spec->catfile( $dataroot, "reduced", "ufti", $ut ),
                             ORAC_PERSON => 'bradc',
                             ORAC_LOOP => 'flag',
                             ORAC_SUN => '232',
                           },
             'UIST'     => { ORAC_DATA_CAL => File::Spec->catfile( $ENV{'ORAC_CAL_ROOT'}, "uist" ),
                             ORAC_DATA_ROOT => $dataroot,
                             ORAC_DATA_IN => File::Spec->catfile( $dataroot, "raw", "uist", $ut ),
                             ORAC_DATA_OUT => File::Spec->catfile( $dataroot, "reduced", "uist", $ut ),
                             ORAC_PERSON => 'bradc',
                             ORAC_LOOP => 'flag',
                             ORAC_SUN => '232,236,246',
                           },
