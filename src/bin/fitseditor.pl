...
my ( $update_window, $rebuild_widgets );
...
$menus[2]->menu()->add( "command", -label => "About FITS Editor",
                        -command => [ \&editor_about, $VERSION, $MW ] );
...
$menus[0]->menu()->add( "command", -label => "Open File",
                        -command => sub { } );
...
# Declare variables
my ( $file_select, @file_list );

# Frame for file panel
my $left_frame = $MW->Frame( -relief      =>'flat', 
                               -borderwidth => 1 );
			       
# Listbox for file selection
my $left_scrollbar = $left_frame->Scrollbar();
my $files = $left_frame->Listbox( -borderwidth         => 1,
                                  -selectbackground    => 'blue',
				  -selectforeground    => 'white',
				  -selectmode          => 'single',
				  -font                => $FONT,
...
$left_frame->bind("Listbox", "<2>",['scan','mark',Ev('x'),Ev('y')]);
$left_frame->bind("Listbox", "<B2-Motion>",['scan','dragto',Ev('x'),Ev('y')]);
...
my $right_frame = $MW->Frame( -relief      =>'flat', 
                               -borderwidth => 1 );
...
$directory_entry->bind( "<Return>", 
     sub { unless ( opendir ( DIR, $working_directory ) ) {
	        print "Directory $working_directory not found\n";
		return; };
           @file_list = ();
...
$files->bind( "<ButtonPress>", 
		 ...
		 if ( defined $header ) {
		    ...
		    $FILE_TYPE = $FILE_TYPE . ", Cards: " . $#cards;
		      ...
		      $header_value, $header_comment ) =
	                 editor_display_header( \@cards, $keyword_widget );
		    ...
		    $MW->update;
		          ...
		          print "Setting header $i to have value $$header_value[$i]\n";
	                  ...
	                  print "Setting header $i to have comment $$header_comment[$i]\n";
	    ...
	    elsif ( -d File::Spec->catfile($working_directory,$$file_select[0]))
	    {
		 ...
		 unless ( opendir ( DIR, $new_directory ) ) {
	           print "Directory $new_directory not found\n";
		   return; };
...
$add_button->bind( "<ButtonPress>",
	 ...
	 if ( $insert_flag eq "plain" ) {
	    $after_index = 0; $insert_flag = "index";
...
# pack the file frame
$left_frame->grid( -column => 0, -row => 2, -sticky => 'nsew' );

# pack the keyword frame
$right_frame->grid( -column => 1, -row => 2, -sticky => 'nsew' );
