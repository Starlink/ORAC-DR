...
# from /home/timj/oracdr/gui/sigma.ui
...
sub sigma_ui {
	...
	my($suffixentry) = $root->Entry (
		-textvariable => \$STATUS{SIGMA}{SUFFIX},
		-width => '8',
	);
	...
	my($toolmenu) = $root->Menubutton (
		-relief => 'ridge',
		-text => 'no',
		-textvariable => \$STATUS{SIGMA}{TOOL},
		-width => '10',
	);
	...
	my($windowmenu) = $root->Menubutton (
		-relief => 'ridge',
		-text => 'no',
		-textvariable => \$STATUS{SIGMA}{WINDOW},
	);
	...
	my($regionmenu) = $root->Menubutton (
		-relief => 'ridge',
		-text => 'no',
		-textvariable => \$STATUS{SIGMA}{REGION},
	);
	...
	my($entry_2) = $root->Entry (
		-textvariable => \$STATUS{SIGMA}{RANGE},
		-width => '12',
	);
	...
	my($entry_3) = $root->Entry (
		-textvariable => \$STATUS{SIGMA}{DASHED},
		-width => '12',
	);
	...
	$button_8->configure(
		-command => sub { modify_current('SIGMA') }
	);
	$button_4->configure(
		-command => sub { &set_default_status('SIGMA');}
	);
	$button_7->configure(
		-command => sub { add_entry('SIGMA') }
	);
	...
	$frame_2->gridColumnconfigure(4, -weight => 0, -minsize => 47);
	$frame_2->gridColumnconfigure(5, -weight => 0, -minsize => 2);
