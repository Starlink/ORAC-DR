...
=item ORAC::Error::TermProcessing

Terminate processing of this recipe but continue on as if everything
is fine. This can be used to stop a recipe early but will not
trigger any long term errors to propagate through the pipeline.

=item ORAC::Error::TermProcessingErr

As for TermProcessing except that whilst the pipeline will continue
to run the exit status of the pipeline will be non-zero to indicate
that a problem was found. This can be used when the pipeline can
work around the problem but would like the final exit status to
reflect that there is a problem to be investigated.
...
package ORAC::Error::TermProcessingErr;
use base qw/ ORAC::Error::TermProcessing /;
