...
sub orac_calc_instrument_settings {
  ...
  my %default_envs = ( 'ACSIS'  => { code => sub { $acsis_con->( "jcmtdr\@jach.hawaii.edu", "XXX" ); },
                       ...
                       'LCOSBIG' => { code => sub { $ukirt_con->( "lcosbig", "tlister\@lcogt.net", "XXX" ) },
                                  ORAC_DATA_IN => File::Spec->catfile( $dataroot, "kb76", $options{ut}, "raw" ),
                                  ORAC_DATA_OUT => File::Spec->catfile( "/mnt/images/daydirs/", "kb76", $options{ut}, "oracproc" ),
                                  ORAC_LOOP => "flag -skip",
                                  args => { $ukirt_args->() }, },
                       'LCOCC' => { code => sub { $ukirt_con->( "lcocc", "tlister\@lcogt.net", "XXX" ) },
                                  ORAC_DATA_IN => File::Spec->catfile( $dataroot, "kb22", $options{ut}, "raw" ),
                                  ORAC_DATA_OUT => File::Spec->catfile( "/mnt/images/daydirs/", "kb22", $options{ut}, "oracproc" ),
                                  ORAC_LOOP => "flag -skip",
                                  args => { $ukirt_args->() }, },
