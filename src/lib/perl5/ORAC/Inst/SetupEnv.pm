...
#    my @eng = ($options{eng} ? ("eng") : ());
  ...
  my %default_envs = ( 'ACSIS'  => { code => sub { $acsis_con->( "jcmtdr\@jach.hawaii.edu", "XXX" ); },
                       ...
                       'LCOSBIG' => { code => sub { $lcogt_con->( "lcosbig", "tlister\@lcogt.net", "XXX", $sitecode, $cameracode ) },
...
#                                  ORAC_DATA_OUT => File::Spec->catfile( "/mnt/images/daydirs/", "kb73", $options{ut}, "oracproc" ),
                                  ORAC_LOOP => "flag -skip",
                                  args => { $ukirt_args->() }, },
                       'LCOCC' => { code => sub { $lcogt_con->( "lcocc", "tlister\@lcogt.net", "XXX", $sitecode, $cameracode ) },
#                                  ORAC_DATA_IN => File::Spec->catfile( $dataroot, $sitecode, $cameracode, $options{ut}, "raw" ),
#                                  ORAC_DATA_OUT => File::Spec->catfile( "/mnt/images/daydirs/", $cameracode, $options{ut}, "oracproc" ),
                                  ORAC_LOOP => "flag -skip",
                                  args => { $ukirt_args->() }, },
                       'LCOSBIG_0M4' => { code => sub { $lcogt_con->( "lcosbig_0m4", "tlister\@lcogt.net", "XXX", $sitecode, $cameracode ) },
