...
sub orac_calc_instrument_settings {
  ...
  my %default_envs = ( 'ACSIS'  => { code => sub { $acsis_con->( "b.cavanagh\@jach.hawaii.edu", "XXX" ); },
                                     ORAC_DATA_IN => File::Spec->catfile( $dataroot, "raw", "acsis", "spectra", $options{ut} ),
                                     ORAC_LOOP => "flag -skip",
                                     args => { $jcmt_args->() },
                                   },
                       'CGS4'   => { code => sub { $ukirt_con->( "cgs4", "b.cavanagh\@jach.hawaii.edu", "236" ) },
                                     ORAC_LOOP => "flag -skip",
                                     args => { $ukirt_args->() },
                                   },
                       ...
                       'CURVE'    => { code => sub { $ukirt_con->( "curve", "b.cavanagh\@jach.hawaii.edu", "236" ) },
                       ...
                       'IRCAM'  => { ORAC_DATA_CAL => File::Spec->catfile( $env{'ORAC_CAL_ROOT'}, "ircam" ),
                                     ...
                                     ORAC_PERSON => 'b.cavanagh\@jach.hawaii.edu',
                                     ORAC_SUN => '232',
                                     ORAC_LOOP => "wait",
                                     args => { $ukirt_args->() },
                                   },
                       'IRCAM2' => { code => sub { $ukirt_con->("ircam", "b.cavanagh\@jach.hawaii.edu", "232" ) },
                                     ORAC_LOOP => "flag -skip",
                                     args => { $ukirt_args->() }, },
                       ...
                       'ISAAC'     => { code => sub { $ukirt_con->( "isaac", "mjc\@star.rl.ac.uk", "232,236" ) },
                                        ...
                                        args => { $ukirt_args->() }, },
                       # experimental
                       "JCMT_DAS" => { code => sub { $ukirt_con->( "heterodyne", "b.cavanagh\@jach.hawaii.edu", "230" ) },
                                       ORAC_LOOP => "flag -skip",
                                       args => { $oracdr_args->() }, },
                       'MICHELLE' => { code => sub { $ukirt_con->( "michelle", "b.cavanagh\@jach.hawaii.edu", "232,236" ) },
                                       ORAC_LOOP => "flag -skip",
                                       args => { $ukirt_args->() },},
                       ...
                       'NIRI'     => { code => sub { $ukirt_con->( "niri", "p.hirst\@gemini.edu", "XXX" ) },
                                       ...
                                       args => { $ukirt_args->() },},
                       # Old CGS4
                       'OCGS4'    => { code => sub { $ukirt_con->( "cgs4", "b.cavanagh\@jach.hawaii.edu", "236" ) },
                                       ORAC_LOOP => "flag -skip",
                                       args => { $ukirt_args->() }, },
                       ...
                       'SOFI'     => { code => sub { $ukirt_con->( "sofi", "mjc\@star.rl.ac.uk", "232,236" ) },
                                       ...
                                       args => { $ukirt_args->() }, },
                       # New UFTI (non-FITS)
                       'UFTI2'    => { code => sub { $ukirt_con->( "ufti", "b.cavanagh\@jach.hawaii.edu", "232" ) },
                                       ORAC_LOOP => "flag -skip",
                                       args => { $ukirt_args->() }, },
                       'UFTI_CASU' => { code => sub { $ukirt_con->( "ufti", "jrl\@ast.cam.ac.uk", "232" ) },
                                        ...
                                        ORAC_LOOP => "flag -skip",
                                        args => { $ukirt_args->() }, },
                       'UIST'     => { code => sub { $ukirt_con->( "uist", "b.cavanagh\@jach.hawaii.edu", "232,236,246" ) },
                                       args => { $ukirt_args->() },
                                       ORAC_LOOP => "$env{ORAC_LOOP} -skip",
                                     },
                       'WFCAM'  => { code => sub { $wfcam_con->( "b.cavanagh\@jach.hawaii.edu", "XXX" ) },
                                     ORAC_LOOP => "flag",
                                     args => { $ukirt_args->() },
                                   },
                    );
