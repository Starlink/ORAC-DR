...
sub subfrms {
  my $self = shift;
  my %related = $self->files_from_hdr(@_);

  my @subfrms;
  for my $key (keys %related) {
    my $frm = $self->new();
...
sub files_from_hdr {
  ...
  if ( exists $self->hdr->{SUBHEADERS} ) {
    ...
    my @files = $self->files;
    if (@subhdrs != @files) {
      croak "files_from_hdr called with number of files (".@files.") different from number of subheaders (".@subhdrs.")";
    }
  }
