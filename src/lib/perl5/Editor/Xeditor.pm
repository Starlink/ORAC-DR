...
sub editor_about {
  ...
  croak 'Usage: editor_about( $editor_version, $MW )'
    unless scalar(@_) == 2 ;

  my ( $editor_version, $MW ) = @_;
...
sub editor_open_header {
  ...
  croak 'Usage: $header = editor_open_header($file_select, $working_directory, $FILE_TYPE)'
    unless scalar(@_) == 3;

  my ( $file_select, $working_directory, $FILE_TYPE ) = @_;
  ...
  if( $$file_select[0] =~ ".sdf" ) {
     ...
     # unidentified
     print "File $$file_select[0] has an unknown file type\n";
     return;
  }
...
sub editor_display_header {
  ...
  croak 'Usage: editor_display_header( $cards, $keyword_widget )'
    unless scalar(@_) == 2;

  my ( $cards, $keyword_widget ) = @_;
