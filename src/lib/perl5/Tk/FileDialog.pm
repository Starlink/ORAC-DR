...
#  Purposes:
#    A reuseable File Dialog (ORAC-DR style)

#  Description:
#    A reusable FileDialog based on Brent Powers original code distributed
#    with Perl::Tk. The Dialog has been modified to fit in the Xoracdr and
#    FITS Editor style.
...
#    MKR: Martin Kraegeloh (mkr@dm-server.cv.com)
#    AA:  Alasdair Allan (aa@astro.ex.ac.uk)
...
#        FileListBox to "background color" in case of -SelDir option.
#   14-Sep-01 AA:
#       version 1.4.2: Added -Font configuration option
#   01-Oct-01 AA:
#       version 1.4.3: Bug fix for font sizes in some widgets
...
display of .* files and directories), and three buttons, namely OK, Rescan, and
Cancel.  Note that the labels for all subwidgets (including the text for the
buttons and Checkbox) are configurable for foreign language support. The
Listboxes will respond to characters typed over them with scrolling to the
first line that starts with the given character (or next etc. if this character
is not present).
...
Enable the user to specify a file that does not exist. If not enabled, and the
user specifies a non-existent file, a dialog box will be shown informing the
user of the error (This Dialog Box is configurable via the EDlg* switches,
below).
...
Disables the ability of the user to change the status of the ShowAll flag. The
default is 0 (the user is by default allowed to change the status).
...
Changes the font style of the widget, defaults to 8pt Helvetica.
...
Enables the File Dialog to do an application Grab when displayed. The default
is 1.
...
True sets the File List box to be to the right of the Directory List Box. If 0,
the File List box will be below the Directory List box. The default is 1.
...
Sets the geometry of the File Dialog. Setting the size is a dangerous thing to
do. If not configured, or set to '', the File Dialog will be centered.
...
SelHook is configured with a reference to a routine that will be called when a
file is chosen. The file is called with a sole parameter of the full path and
file name of the file chosen. If the Create flag is disabled (and the user is
not allowed to specify new files), the file will be known to exist at the time
that SelHook is called. Note that SelHook will also be called with directories
if the SelDir Flag is enabled, and that the FileDialog box will still be
displayed. The FileDialog box should B<not> be destroyed from within the
...
returns non-zero, the FileDialog will immediately be withdrawn, and the file
...
routine with 0 removes the SelHook routine. The default SelHook routine is
undef.
...
sub Populate {
    ...
    $FDialog->{Configure}{-Font} = 'Helvetica 8';
